...
> **상태: 지금 시점 참고용 — 계획대로 되지 않았다.** 백엔드 Plan 2~3과 병렬로 진행하려던 UX
> 설계 트랙이었으나, 아래 `docs/design/wireframes/`에 `.pen` 파일을 두는 계획은 실행되지
> 않았다(디렉터리가 끝까지 비어 있었다). 실제 시각 설계는 나중에 `docs/design/kaldi-note-design/`
> 의 `.dc.html` 캔버스 방식으로 대체됐다.
>
> 이 문서는 **화면 목록·흐름·도메인 규칙 반영 지점**을 텍스트로 고정한다. `.pen`은 바이너리라
> git diff 리뷰가 안 되므로, **레이아웃 결정의 이유는 항상 이 문서(마크다운)에 먼저 적는다**는
> 원칙 자체는 여전히 유효하다.
