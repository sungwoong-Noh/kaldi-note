...
- [x] Swagger UI에서 실제 로스터·블렌드 원두 상품을 등록하고, 재고를 만들어 `daysOffRoast`·`degassingStatus`가 오늘 날짜 기준으로 맞는지 확인

> **2026-09-03 확인.** 로스터(`확인용 로스터`)와 **블렌드** 상품(`확인용 블렌드`, 에티오피아 60% + 콜롬비아 40%)을 실제로 등록하고, 오늘(2026-09-03) 기준으로 **경계값 네 개**를 노려 재고를 만들었다. 기대값을 먼저 적고 응답과 대조했다.
>
> | `roastedAt` | 기대 | 실제 | |
> |---|---|---|---|
> | 2026-09-01 | 2 · `TOO_FRESH` | 2 · `TOO_FRESH` | ✅ |
> | 2026-08-31 | 3 · `IDEAL` | 3 · `IDEAL` | ✅ |
> | 2026-08-20 | 14 · `IDEAL` | 14 · `IDEAL` | ✅ |
> | 2026-08-19 | 15 · `PAST_PEAK` | 15 · `PAST_PEAK` | ✅ |
>
> 4/4 일치. `TOO_FRESH`↔`IDEAL`(2↔3)과 `IDEAL`↔`PAST_PEAK`(14↔15) **양쪽 경계를 모두 밟았다.**
>
> **`origins`의 필드명은 `country`다**(`countryCode`가 아니다). `countryCode`로 보내면 `origins[0].country: 공백일 수 없습니다`가 나온다 — 이름만 보고 짐작하면 틀린다.
