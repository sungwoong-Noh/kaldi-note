...
title: 사진 첨부
status: 구현완료
plan: docs/archive/plans/2026-08-18-plan-media.md
---
...
- [x] ★ 실제 OCI Object Storage 크리덴셜로 업로드 URL 발급 → PAR로 실제 파일 업로드 → 확정 → 공개 URL로 이미지가 실제로 열리는지 확인한다(배포 이후, 로컬 개발에서는 `ObjectStorageClient` 인터페이스를 가짜 구현으로 대체하므로 실제 OCI 연동은 이 단계에서 처음 검증된다)

> **2026-09-06 확인 — 차단형이 풀렸다. 운영에서 전 구간을 밟았다.**
>
> `RECIPE` 대상 하나를 만들고 `POST /attachments/upload-url`로 **PAR을 실제로 발급받았다**
> (`expiresAt`이 10분 뒤로 찍힌다 — 운영에서 OCI SDK 호출이 성공한다는 뜻이다). 그 URL에
> **백엔드를 거치지 않고** 1×1 JPEG를 `PUT`해 `200`, 확정하니 `contentType`이 `image/jpeg`로
> 돌아왔다 — **클라이언트가 보낸 값이 아니라 OCI HEAD에서 읽은 값이다.**
>
> **공개 URL을 인증 없이 GET해 `200 image/jpeg`**를 받았고, **올린 바이트와 내려받은 바이트가
> 같은 것까지 대조했다**(`cmp`). 200만 보면 다른 파일이 와도 통과한다.
>
> 첨부를 지우니 `204`가 나고 **같은 공개 URL이 `404`가 됐다** — DB 행만이 아니라 OCI 객체까지
> 지워진다. 확인용 레시피도 지웠다.
>
> **로컬은 `ObjectStorageClient`를 가짜 구현으로 대체하므로 이 경로는 오늘 처음 실제로 돌았다.**
> (2026-08-19에 사람이 카카오 실계정으로 같은 구간을 밟은 기록이 `infra/README.md`에 있다.)
>
> <details><summary>이전 기록 (2026-09-05)</summary>
>
> **★ 이 스펙은 `구현완료`이지만 차단형 확인이 1개 남아 있다.** 규약
