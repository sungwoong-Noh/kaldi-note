 ...
 * 레시피 목록 화면 본문.
 *
 * <p><b>`useSearchParams`를 쓰지 않는다.</b> 클라이언트에서 쿼리를 읽으면 Suspense 경계를
 * 요구해서 프로덕션 빌드의 프리렌더가 깨진다(2026-09-17에 겪었다 — `/brews/new`가
 * 2026-08-30에 겪은 것과 같은 함정이다). 쿼리는 서버 컴포넌트가 풀어 prop으로 넘긴다.
 */
export interface GrindHandoff {
  grinderModelId?: number;
  grindSettingValue?: number;
}

export function RecipeListScreen({ grind }: { grind: GrindHandoff }) {
  /*
   * 환산기에서 분쇄도를 들고 들어온 경우다. 카드를 누르면 상세가 아니라
   * 기록 작성으로 보내고, 그 값을 그대로 넘긴다
   * (docs/specs/2026-09-17-small-features.md).
   */
  const handingGrind =
    grind.grinderModelId !== undefined && grind.grindSettingValue !== undefined;

  const brewHref = (recipeId: number): string | undefined =>
    handingGrind
      ? `/brews/new?recipeId=${recipeId}&grinderModelId=${grind.grinderModelId}&grindSettingValue=${grind.grindSettingValue}`
      : undefined;
