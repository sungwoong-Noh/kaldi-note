...
- [x] **Step 5: 스펙 status를 올린다** — **이 세션에는 하지 않았다(2026-09-05). 차단형 3개가 남아 의도적으로 `초안`으로 뒀다.** **그 뒤 2026-09-06 검증 세션에서 운영에 시크릿을 넣고 실제로 로그인해 차단형 셋을 밟은 뒤 `구현완료`로 올렸다(PR #106).**
...
- [x] 스펙 「수동 확인」 5개 — 차단형 3개를 2026-09-06에 운영에서 밟았고, 그때 `status`를 `구현완료`로 올렸다. 남은 1개(「`.env`에서 그 줄을 지울지 판단한다」)는 비차단형이며 아직 열려 있다
