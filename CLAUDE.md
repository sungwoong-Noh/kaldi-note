...
> 인수 조건 508개가 초록이다.
>
> **남은 것은 코드가 아니라 「수동 확인」 31개다.** 대부분 폰 실물 조작이라 사람만 밟을 수 있고,
> 그중 12개가 스펙 3개를 `초안`에 묶어 둬 **AC 96개가 커버리지 검사에서 빠져 있다.**
> 현황은 `docs/JOURNAL.md` 최신 항목과 `docs/conventions/verification.md`를 본다.
...
│   ├── specs/                ★ 기능 스펙 — AC를 가진 문서만 여기 둔다 (22개, AC 604개)
│   │   ├── TEMPLATE.md
│   │   └── YYYY-MM-DD-<기능>.md   날짜순. 셋은 아직 `초안`이다(수동 확인이 남아서)
