...
- [x] ★ **운영에서 둘이 실제로 맞팔로우가 된다.** 한쪽이 「더보기」에서 링크를 복사해 카톡으로 보내고, 상대가 열어 팔로우한 뒤, 반대로도 한다. **이 스펙이 존재하는 이유이고 자동 테스트는 실계정 둘을 만들 수 없다**
- [x] ★ 맞팔로우가 된 뒤 상대의 `FRIENDS` 레시피가 내 `/recipes` 목록에 실제로 나타난다
- [x] 폰에서 「복사」가 실제로 클립보드에 들어가고 카톡에 붙여넣어진다 (iOS 사파리는 클립보드 권한이 다르다)

> **2026-09-06 확인 — 차단형 둘을 운영에서 밟았고 `구현완료`로 올렸다.** 테스트 로그인으로
> `확인용가`(id 3)와 `확인용나`(id 5)를 만들어 두 브라우저 컨텍스트로 번갈아 밟았다.
>
> **양방향을 따로 봤다.** 확인용나가 `/u/3`을 열었을 때 버튼은 「팔로우」였고, 누른 뒤에는
> 「팔로우 취소」 + 「상대도 나를 팔로우하면 서로의 기록이 보입니다」였다 — **아직 맞팔로우가 아니라는
> 것을 화면이 정확히 구분했다.** 반대로 확인용가가 `/u/5`에서 팔로우하자 그 자리에서
> 「맞팔로우 — 서로의 기록이 보입니다」로 바뀌었고, **확인용나 쪽에서 페이지를 새로 열어도** 같은
> 문구가 유지됐다(양방향이 DB에 남았다는 뜻이다).
>
> **`FRIENDS` 노출은 음성 대조군을 함께 뒀다.** 확인용가가 레시피 둘을 만들었다 —
> `FRIENDS`인 「확인용 맞팔공개」와 `PRIVATE`인 「확인용 비공개」. 확인용나의 `/recipes` 목록에는
> **앞의 것만** 떴다. 「보인다」만 봤으면 인가가 통째로 열려 있어도 같은 그림이었을 것이다.
> API로도 대조했다 — 확인용나가 받는 6개는 맞팔로우 상대의 `FRIENDS` 하나와 `PUBLIC` 다섯뿐이고,
> 운영 실계정(id 1·2)의 비공개는 하나도 새지 않는다.
>
> **폰에서 「복사」도 실제로 붙여넣어졌다**(2026-09-06, 사람이 폰 브라우저로 확인). 이 항목은
> 비차단형이었지만 같은 자리에서 함께 밟혔다.
>
> **확인용 레시피 둘은 확인이 끝난 뒤 지웠다**(`DELETE` 204 · 확인용가의 레시피 0개).
> 계정과 팔로우 관계는 남겼다 — 다음 사람이 `FRIENDS`를 다시 밟을 때 상대를 새로 만들지 않아도 된다.
>
> **남은 하나는 폰 실물이 있어야 한다**(클립보드 복사 → 카톡 붙여넣기). 비차단형이라 `status`를
> 막지 않는다. 위 확인은 운영 도메인에 대고 했지만 데스크톱 크로미움을 390×844로 띄운 것이라
> **「복사」 버튼을 실제로 누른 것은 아니다.**
