...
> 밟았다 — 테스트 로그인·맞팔로우·사진 업로드 OCI 실연동·백업·방화벽·인증서·**자동 롤백**.
> 남은 21개는 폰 조작감이나 문구처럼 `status`를 막지 않는 것들이다
...
> 21개는 폰 조작감·문구처럼 `status`를 막지 않는 것들이다. **여전히 한 번도 확인된 적 없는 것은
> 구글 로그인 하나다**(`oci-deploy`의 해당 상자 근거는 카카오뿐이다).
