...
describe('LoginPage', () => {
  it('AC-WEB-03 · 카카오 로그인이 카카오 인가 URL을 가리킨다', async () => {
    render(await LoginPage({ searchParams: Promise.resolve({ next: '/recipes/1' }) }));
  ...
  it('next가 없으면 목록으로 돌아가도록 한다', async () => {
    ...
    const href = screen.getByRole('link', { name: '카카오로 로그인' }).getAttribute('href') ?? '';

    expect(href).toContain(`state=${encodeURIComponent('/recipes')}`);
  });

  it('외부 주소를 next로 넘겨도 앱 안으로만 돌아간다', async () => {
    render(await LoginPage({ searchParams: Promise.resolve({ next: '//evil.example/steal' }) }));
