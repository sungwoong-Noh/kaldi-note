...
> 인수 조건 508개가 초록이다.
>
> **남은 것은 코드가 아니라 「수동 확인」 31개다.** 대부분 폰 실물 조작이라 사람만 밟을 수 있다.
> 그중 **차단형(`★`)이 7개** — 백업 4개는 배포 이후 지금까지 한 번도 검증된 적 없는 재해 복구다.
> 나머지 24개는 비차단형이라 `status`를 막지 않는다(`docs/conventions/verification.md`).
...
> **주의:** 배포된 것이 곧 검증된 것은 아니다. **수동 확인 31개가 남아 있고** 그중 8개는 운영·인프라
> (백업이 실제로 도는지 아무도 확인한 적 없다), 23개는 폰 실물 조작이다.
