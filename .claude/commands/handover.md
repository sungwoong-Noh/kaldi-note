...
이번 세션이 어느 쪽이었는지 먼저 정하라. 이후 단계가 달라진다.

| 세션 | 어떻게 아는가 | 2단계(체크박스) | 6단계(PR) |
|---|---|---|---|
| **구현** | `backend/`·`frontend/` 코드를 고쳤다 | 한다 | 태스크 완료 시 |
| **설계** | `docs/specs/`·`docs/plans/`만 고쳤다 | **건너뛴다** | 스펙·계획이 승인되면 |
| **디자인** | `docs/design/`만 고쳤다 | **건너뛴다** | 문서가 정리되면 |
...
설계·디자인 세션이라면 코드 검증은 해당 없고 `check-spec-coverage.sh`만 돌리면 된다. 스펙을 새로 썼다면 `status: 초안`이라 건너뛰는 게 정상이다.

## 2단계: 계획 체크박스 갱신 (구현 세션만)

`docs/plans/`의 해당 계획 문서에서 **이번 세션에 실제로 완료한 Step**만 `- [ ]` → `- [x]`로 바꾼다.

- 태스크의 모든 Step이 끝나야 그 태스크가 완료다
- **중간에 멈췄으면 거기까지만 체크한다.** 앞당겨 채우지 않는다
- **설계 세션은 체크박스를 건드리지 않는다.** 본문만 고친다
