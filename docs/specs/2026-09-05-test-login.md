...
- **Then** 기존 카카오 로그인 테스트가 초록이다
- **검증** 통합 테스트 `AuthServiceTest.처음_로그인하면_사용자가_생성된다`

> **2026-09-06에 이 AC의 문구를 고쳤다.** 원래는 「기존 `AC-AUTH-*` 테스트가 전부 초록이다」였는데,
> **`AC-AUTH-*`라는 ID는 저장소에 존재한 적이 없다**(인증은 스펙 문서 없이 만들어진 Foundation
> 산출물이다). 검증 방법이 「기존 테스트 재실행」이라 **어떤 테스트에도 ID가 남지 않았고**, 스펙이
> `초안`이라 커버리지 검사가 통째로 건너뛰어 아무도 몰랐다. `초안`이 가리고 있던 실제 구멍이다 —
> `구현완료`로 올리는 순간 빨개졌다. 지금은 그 조건이 실제로 검사하는 테스트에 ID를 붙였다.
...
- [x] ★ **운영 `.env`에 `KALDI_TEST_LOGIN_SECRET`을 32자 이상으로 채우고 배포한다.** 값은 비밀번호 관리자에 둔다 — 이 값이 곧 모든 계정의 열쇠다
- [x] ★ 폰에서 `https://kaldi-note.today/login/test`로 실제 로그인이 되고, 「더보기」에 닉네임이 뜬다
- [x] ★ 계정 둘을 만들어(`handle`을 둘로) 서로 팔로우해 맞팔로우가 된다 — `web-follow` 스펙의 차단형 확인이 이걸로 풀린다
- [x] 시크릿을 틀리게 넣으면 `테스트 로그인을 쓸 수 없습니다`가 뜬다
- [ ] **다 쓰고 나면 `.env`에서 그 줄을 지울지 판단한다.** 만료가 없으므로 지우는 것 말고 꺼지는 방법이 없다

> **2026-09-06 확인.** 운영 `.env`에 64자 시크릿을 넣고 **돌던 이미지 태그를 그대로 고정해**
> 재기동했다(`ed6e94c` 유지 — 검증 때문에 버전이 움직이면 무엇을 본 것인지 알 수 없어진다).
>
> **기능이 켜졌다는 것을 「켜지기 전」과 대조해서 봤다.** 시크릿을 넣기 전 `POST /api/v1/auth/login/test`는
> `404 ENDPOINT_NOT_FOUND`였고, 넣은 뒤에는 **틀린 시크릿·헤더 없음은 여전히 404**인데 **맞는 시크릿에만**
> `400 INVALID_REQUEST`(`"userId와 handle 중 정확히 하나를 준다."`)가 돌아온다. 없는 경로와 구별되지
> 않아야 한다는 요구가 운영에서 실제로 지켜진다.
>
> **`TEST` 계정 셋이 실제로 생겼다** — `확인용가`(id 3) · `확인용프로브`(id 4) · `확인용나`(id 5).
> `handle`로 부르면 `newUser: true`로 만들어지고, 같은 `userId`로 다시 부르면 그 계정을 그대로 쓴다.
> 운영에는 이미 실계정 둘(id 1·2)과 그들의 레시피가 있었고, `TEST` 계정과 섞이지 않았다.
>
> 시크릿을 틀리게 넣으면 화면에 빨간 「테스트 로그인을 쓸 수 없습니다」가 뜨고 로그인되지 않는다.
> 그 요청으로 `should-not-exist` 계정도 생기지 않는다(백엔드가 404를 내므로 생성 경로에 닿지 않는다).
>
> **폰 실물로도 밟았다.** 위 확인은 데스크톱 크로미움을 390×844로 띄운 것이었지만, 그 뒤 사람이
> 폰 브라우저로 `사용자 id 3`을 넣어 로그인해 **「더보기」에 「확인용가」가 뜨는 것을 확인했다.**
> 같은 화면에서 「복사」도 실제로 클립보드에 들어가 카톡에 붙여넣어졌다
> (`web-follow`의 비차단형 항목이 이걸로 함께 풀렸다).
>
> **★ 시크릿은 지금 운영에 살아 있다.** 마지막 항목(지울지 판단)의 상자를 비워 둔 이유다 —
> 2026-09-06에는 **남기기로 판단했다.** 다음 확인 때 계정을 다시 만들지 않아도 되고, `TEST` 계정
> 셋과 맞팔로우 관계가 그대로 남아 있어서다. 지울 때는 `.env`에서 그 줄을 지우고 재기동하면
> 경로가 다시 404가 된다.
