...
- [ ] **Step 2: 실패 확인** — Expected: FAIL(모달 진입 버튼 없음)
- [ ] **Step 3: 최소 구현** — 모달 열림 상태를 페이지가 들고, 폼 상태는 그대로 둔다. 등록 성공 시 해당 선택란의 값을 새 id로 바꾸고 목록 쿼리를 무효화한다.
- [ ] **Step 4: 통과 확인** — Expected: PASS, 5 tests
- [ ] **Step 5: 커밋** — `feat(web): 작성 화면의 선행 데이터 모달 (AC-WEBBREW 5개)`
