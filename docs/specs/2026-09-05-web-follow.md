...
> **코드는 2026-09-06에 완성됐지만 `status`는 `초안`으로 둔다.** 차단형(`★`) 둘이 남아 있고,
> `docs/conventions/verification.md`는 「차단형이 하나라도 남으면 올리지 않는다」이다. **이 스펙이
> 존재하는 이유가 바로 그 둘이라** 밟기 전에 올리면 검사가 아니라 자기기만이 된다.
>
> **이제는 밟을 수 있다.** 2026-09-05의 테스트 로그인(`2026-09-05-test-login.md`)이 `handle`로 계정을
> 만들 수 있게 했다 — 운영 `.env`에 `KALDI_TEST_LOGIN_SECRET`을 넣으면 계정 둘을 만들어 서로
> 팔로우해 볼 수 있다. 그 뒤에 `구현완료`로 올린다.
