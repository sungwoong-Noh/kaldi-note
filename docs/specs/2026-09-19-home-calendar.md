...
- 하단 CTA 「기록하기」 → `/brews/new`. **선택된 날짜가 프리필된다.**
...
| 자리 | 내 달력 | 남의 달력 |
|---|---|---|
| 상단 바 (전역) | primary `기록하기` → `/brews/new` (**날짜 프리필 없음**) | 같음 |
| 우측 하단 (선택 날짜 종속) | primary `이 날짜로 기록 추가` → `/brews/new?date=2026-09-19` | secondary `지연 님 프로필 보기` → `/u/12` |
...
#### AC-HOMECAL-60 · 상단 바 CTA는 날짜를 프리필하지 않는다

- **Given** 뷰포트 폭 `1440px`, `2026-09-05`가 선택된 상태
- **When** `기록하기`를 누른다
- **Then** 주소가 `/brews/new`다 — `?date=`가 붙지 않는다
- **검증** e2e `home-calendar-web.spec.ts`

#### AC-HOMECAL-61 · 우측 하단 CTA는 선택 날짜를 프리필한다

- **Given** 뷰포트 폭 `1440px`, 내 달력에서 `2026-09-05`가 선택된 상태
- **When** `이 날짜로 기록 추가`를 누른다
- **Then** 주소가 `/brews/new?date=2026-09-05`다
- **검증** e2e `home-calendar-web.spec.ts`
...
#### AC-HOMECAL-48 · 모바일 하단 CTA는 선택 날짜를 프리필한다
...
- [ ] 1440px에서 셀의 레시피명 한 줄이 실제 레시피 제목 길이로 ellipsis 되는가
