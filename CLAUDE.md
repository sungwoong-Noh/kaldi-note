...
> **백엔드 503개 · 프론트 312개 · e2e 65개 · 인수 조건 678개**가 초록이다(2026-09-06).
>
> **★ 구현할 계획이 하나도 남지 않았다.** `docs/plans/`의 26개가 전부 구현됐다. 새 기능은 `/interview`로
> 시작한다 — 「이어서 할 태스크」는 더 이상 없다.
>
> **★ 스펙 26개가 전부 `구현완료`다. `초안`이 하나도 없다.** 2026-09-06에 운영에서 테스트 로그인과
> 맞팔로우를 밟아 마지막 둘(`test-login`·`web-follow`)을 올렸다. **테스트 로그인 시크릿은 지금 운영에
> 살아 있다** — 끄려면 `.env`에서 그 줄을 지우고 재기동한다.
>
> **남은 것은 코드가 아니라 「수동 확인」 22개이고, 그중 차단형(`★`)은 단 1개다** — 헬스체크를 일부러
> 실패시켰을 때 직전 태그로 자동 롤백되는지(`oci-deploy`). **운영을 의도적으로 깨뜨리는 실연습**이라
> 별도 세션으로 둔다. 나머지 21개는 비차단형이라 `status`를 막지 않는다
> (`docs/conventions/verification.md`).
>
> **백업은 검증됐다.** 2026-09-06에 운영 버킷을 직접 봤다 — `0 3 * * *` cron이 등록돼 있고,
> **08-31~09-06 날짜가 하나도 빠지지 않은 7개**가 있으며 8일 전 것은 지워져 있다. 백업 버킷은
> **실재하는 객체로 확인했을 때 `404`**(같은 조건에서 사진 버킷은 `200`)라 public이 아니다.
> **사진 업로드 OCI 실연동도 전 구간 통과했다**(PAR → PUT → 확정 → 공개 URL → 삭제 후 404).
...
│   ├── specs/                ★ 기능 스펙 — AC를 가진 문서만 여기 둔다 (26개, AC 678개)
│   │   ├── TEMPLATE.md
│   │   └── YYYY-MM-DD-<기능>.md   날짜순. **26개 전부 `구현완료`** — `초안`이 없다
│   ├── plans/                구현 계획 (태스크 단위, 26개 — 전부 구현 완료)
│   │   ├── TEMPLATE.md
...
**기능 단위 스펙 여러 개**로 쪼개져 진행됐다(`docs/specs/`에 26개, `docs/plans/`에 26개).
...
> **주의:** 배포된 것이 곧 검증된 것은 아니다. 다만 2026-09-06에 **운영에서 밟을 수 있는 것은 거의 다
> 밟았다** — 테스트 로그인·맞팔로우·사진 업로드 실연동·백업·방화벽·인증서. **수동 확인 22개가 남았고
> 차단형은 1개**(롤백 실연습)다. 나머지는 폰 조작감처럼 `status`를 막지 않는 것들이다.
