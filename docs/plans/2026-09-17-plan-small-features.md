...
- [ ] 기록 상세에서 차이 문구가 **읽히는 문장인지** — 어색하면 어미를 고친다
      — 스크린샷으로는 「원두를 10.0g 덜 썼습니다」·「8°C 낮게 내렸습니다」로 나온다.
      **폰에서 사람이 최종 확인한다.**
- [x] 환산 → 기록 작성으로 넘어갔을 때 **분쇄도가 실제로 채워져 있는지**
      — **e2e로 확인(2026-09-17).** `/brews/new?recipeId=12&grinderModelId=1&grindSettingValue=30`에서
      `그라인더=2 · 분쇄도값=30`이 채워지고, 값이 없으면 채우지 않는다.
