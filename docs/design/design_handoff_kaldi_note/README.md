...
kaldi-note는 핸드드립 커피의 **추출 변수를 기록하고 가까운 사람과 공유하는** 앱입니다. 이 번들은 브랜드 방향(Clean Ledger / Editorial), 디자인 시스템(색·타입·간격·컴포넌트·보이스), 그리고 모바일 11화면 + 웹 3화면의 하이파이 목업을 담고 있습니다.

핵심 제품 원칙 3가지 — 구현 시 판단 기준으로 쓰세요:

1. **기록(변수 로그)이 1순위**, 친구 공유는 그 결과를 내보내는 두 번째 층.
2. **화면마다 대표 수치 하나**를 36px 모노스페이스로 크게 둡니다(비율 · 수율 · 별점 · 환산값 중 하나).
...
### M4 · 레시피 작성/편집 `/recipes/new`, `/[id]/edit`
