...
`style: 포맷만 적용, 동작은 그대로`라고 써놓고 로직 변경이 섞여 들어간 적이 있다(`docs/JOURNAL.md` 2026-08-21). 스테이징에서 로직 파일을 빼는 명령이 조용히 실패했는데 결과를 대조하지 않고 커밋했다.
...
**태스크마다 브랜치를 만들어 서로를 base로 잡는 구조(스택 PR)는 금지한다.** 이 프로젝트에서 두 번 연속 같은 사고가 났다 (`docs/JOURNAL.md` 2026-08-17 두 항목).
...
**태스크별 추적은 PR이 아니라 계획 문서의 체크박스가 담당한다** — 그게 단일 진실 원천이다. 되돌릴 일이 생기면 커밋 단위 `revert`로 충분하다.
- 머지된 브랜치는 삭제한다.
