...
- **모바일 하단 고정 버튼은 `fixed` + 아래 여백이 아니라 `sticky bottom-0`으로 만든다.** 버튼 줄
  높이만큼 여백을 주려면 간격 6단계 밖의 값(`pb-24` 등)이 필요해 `AC-SPACE-01`이 깨진다. 폼 폭을
  넓힐 때도 `max-w-2xl`을 직접 쓰면 `AC-SKIN-01`이 깨진다 — `Shell wide` + 다른 `max-w-*`로 준다.
- **"화면 아래에서 N px 이내" 같은 e2e는 음수 간격도 통과시킨다.** 요소가 화면 밖(아래)에 있으면
  `뷰포트 높이 - 아래끝`이 음수라 `<= N`을 만족한다. `>= 0`도 함께 단언하고, 구현 뒤에 쓴 레이아웃
  e2e는 해당 CSS를 빼고 실제로 실패하는지 한 번 확인한다.
- **`prettier --write <디렉터리>`는 이번 작업과 무관한 파일까지 고친다** — 저장소가 prettier 기준으로
  완전히 정리돼 있지 않다(예: `src/app/brews/page.tsx`). 고친 파일만 경로로 지정한다.
