...
- [x] TDS를 나중에 넣어 추출 수율·SCA 영역이 새로 나타나는지
- [x] 공개범위를 `맞팔로우만`으로 바꾼 뒤 상호 팔로우 계정에서 보이는지
- [ ] 운영(`kaldi-note.today`)에서 편집·저장이 도는지

> **2026-09-05 확인.** 4개 중 **2개를 켰다.** 남은 둘은 폰 실물과 운영 실계정이다.
>
> **✅ TDS를 나중에 넣으면 없던 절이 생긴다.** TDS 없이 저장한 기록(`/brews/10`)의 상세에는
> 「추출 분석」도 「수율」도 **없었다**(문자열로 확인). 편집에서 음료 중량 260g · TDS 1.40%만 채워
> 저장하니 상세에 `TDS 1.4 % · 수율 18.2 % · 농도 STRONG · 추출 IDEAL`과
> 「농도가 진합니다…」가 새로 나타났다. 수율은 손으로 셈한 값(260 × 1.40 ÷ 20 = 18.2)과 일치한다.
>
> **✅ 맞팔로우만으로 바꾸니 상대에게 보인다.** 상호 팔로우인 계정 11·12로 밟았다.
> `나만 보기`일 때 계정 12는 `이 브루잉 로그를 볼 권한이 없습니다.`를 봤고, 계정 11이 공개 범위를
> `맞팔로우만`으로 바꿔 저장하자 **같은 URL에서 기록이 열렸다.** 이때 레시피와 원두 이름은
> `비공개 레시피`·`비공개 원두`로 마스킹돼 있었고 「푸어 스텝」 절도 없었다 —
> `2026-09-02-web-entity-names`와 `2026-09-03-web-brew-log-steps`가 못박은 그대로다.
> 처음엔 이것을 「안 보인다」로 잘못 읽었는데, **가려진 것은 로그가 아니라 남의 비공개 레시피 이름이다.**
