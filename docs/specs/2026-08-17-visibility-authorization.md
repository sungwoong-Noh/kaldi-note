...
- **브루잉 로그 수정 API.** `visibility`는 생성 시점에만 정한다. 만든 뒤 공개로 전환할 수 없다. `PATCH /brew-logs/{id}`는 후속이다.
  > **2026-08-19 정정:** 이 결정은 뒤집혔다. `docs/specs/2026-08-19-list-query-api.md`가 `PATCH /brew-logs/{id}`로 `visibility` 변경을 허용한다(`AC-BLEDIT-09`). 사용자는 일단 `PRIVATE`로 기록해 두었다가 마음에 드는 결과만 친구에게 여는 방식을 기대하며, 그러지 못하면 로그를 지우고 다시 쓰게 된다는 것이 근거다.
...
- [x] `bootRun` + curl로 두 계정을 만들어 서로 팔로우한 뒤, `FRIENDS` 레시피가 상대 계정에서 `200`으로 열리는 것을 확인한다 (`docs/design/2026-08-14-architecture.md:253`의 핵심 시나리오 6단계)
- [x] 한쪽이 팔로우를 해제한 직후 같은 요청이 `403`으로 바뀌는 것을 확인한다

> **2026-09-03 확인. 아키텍처 문서의 핵심 시나리오가 끝까지 돈다.** 사용자 11(`노성웅`)과 12(`확인용친구`), 상호 팔로우 상태에서:
>
> | 상태 | 요청 | 기대 | 실제 |
> |---|---|---|---|
> | 상호 팔로우 | 12 → `GET /recipes/19` (11의 `FRIENDS`) | 200 | **200** ✅ |
> | 12가 11 언팔로우 | 같은 요청 | 403 | **403** ✅ |
> | 팔로우 복구 | 같은 요청 | 200 | **200** ✅ |
>
