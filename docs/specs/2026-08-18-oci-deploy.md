...
- [x] ★ 헬스체크가 실패하도록 강제했을 때, 직전 `:<git-sha>` 태그로 자동 롤백되고 워크플로가 실패(⨯)로 표시된다
...
> - **카카오 실계정 로그인은 오늘 밟지 않았다** — 2026-08-19의 README 기록에 의존한다.
>
> **★ 2026-09-07에 구글도 밟았다.** 운영에서 실제 구글 계정으로 로그인된다
> (`2026-09-07-google-login.md`). **「provider만 다른 같은 경로라 구글도 될 것」이라는 3주 묵은
> 가정은 참이 아니었다** — 프론트에 진입점이 없었고, 백엔드 `GOOGLE_REDIRECT_URI`가 구글 전용
> 콜백 경로와 어긋나 있어 실제로는 `redirect_uri_mismatch`로 실패했다. **밟아보기 전까지는
> 몰랐다.**
>
> **2026-09-07 — 롤백 드릴을 다시 밟았다. 이 스펙에 미체크가 하나도 남지 않았다.**
>
> **이 상자는 이미 두 번 통과한 것을 3주 넘게 켜지 않고 있었다.** `docs/archive/JOURNAL.md`에
> 2026-08-21(최초)과 2026-08-30(재실행, 판정 4개 + 실서비스 전부 PASS) 기록이 있는데 여기도
> `infra/README.md`도 비어 있었다. 그래서 2026-09-05 세션은 「롤백 강제 테스트는 하지 않기로
> 했다」고 적었고 2026-09-06 세션은 「별도 세션으로 두라」고 적었다 — **세 세션이 연달아 이미
> 끝난 일을 남은 일로 취급했다.**
>
> **그래도 기록을 옮겨 적지 않고 다시 밟았다. 옳은 판단이었다** — `deploy.sh`가 **2026-09-05에
> 바뀌었기 때문이다**(`wait_healthy`가 `INFO_URL`의 `build.commit`까지 대조한다). 마지막 드릴은
> 그 변경 **이전**인 08-30이라 결과가 낡아 있었다. JOURNAL이 스스로 「인프라를 손댈 때마다 다시
> 돌릴 것」이라고 적어 두고도 그 변경 뒤로 돌린 적이 없었다.
>
> 기대값을 먼저 못박고 대조했다. 상태파일 `e9c3a75` · 새 배포 시도 `5ac1e13` · 가짜 서버 첫 응답
> `503` · 60초 실패 → 롤백 → **`헬스체크 통과 (시도 1/12)`** · 판정 4개(종료 코드 1 · 롤백 메시지 ·
> 직전 태그 복귀 · 상태 파일 무오염) 전부 PASS · 실서비스 `UP`. **`12/12`가 나왔다면 2026-08-21의
> 오프바이원 재발이었는데 그렇지 않았다.**
>
> **다운타임은 실제로 있었다.** 실서비스 확인이 `시도 4/12`에 통과했다 — 롤백 컨테이너가 뜨고
> 15초 남짓 응답하지 못했다는 뜻이다. 2026-08-30에 「폴링했더니 내내 200이라 안 끊긴 줄 알았다」는
> 기록이 있는데, **이번 출력이 그 폴링이 왜 틀렸는지를 보여준다.**
>
> **「워크플로가 실패(⨯)로 표시된다」는 직접 보지 않았다.** 드릴은 VM에서 `deploy.sh`를 직접
> 부르고, GitHub Actions를 거치지 않는다. 다만 워크플로는 이 스크립트를 `ssh-action`으로 실행하고
> **종료 코드 1이 확인됐으므로** 스텝은 실패한다 — 관측이 아니라 추론이라는 것을 적어 둔다.
>
> **드릴이 밟지 못하는 경로가 하나 생겼다.** 가짜 서버가 12번 내내 `503`이라 「UP이지만 아직 이전
> 버전」 분기에 도달하지 못한다. 그 경로는 `infra/scripts/deploy.test.sh`가 `build-info` 스펙의
> 헬스 대기 AC 다섯(06~10번)으로 덮는다. 스크립트 주석에도 적었다.
>
> **여기에 남의 AC ID를 그대로 쓰지 않는 이유가 있다.** `check-spec-coverage.sh`는 스펙 본문에서
> ID 패턴을 긁어 **그 스펙의 AC로 집계한다.** 인용하려고 `build-info`의 ID를 적었더니 이 스펙의
> AC가 2개에서 3개로 늘고 전체가 678에서 679가 됐다 — **남의 AC를 인용만 해도 내 것이 된다.**
> (고치면서 설명문에 그 ID를 또 적어 같은 함정을 그 자리에서 한 번 더 밟았다.)
>
> **롤백 후보 태그를 최신부터로 바꿨다.** 예전 목록은 3주 전 이미지 셋이었는데, 이 태그는 잠깐이라도
> 실제로 운영에 뜨므로 한 커밋 전 이미지와 위험이 전혀 다르다.
