...
> **백엔드 503개 · 프론트 312개 · e2e 65개 · 인수 조건 637개**가 초록이다(2026-09-06).
>
> **★ 구현할 계획이 하나도 남지 않았다.** `docs/plans/`의 24개가 전부 구현됐다. 새 기능은 `/interview`로
> 시작한다 — 「이어서 할 태스크」는 더 이상 없다.
>
> **남은 것은 코드가 아니라 「수동 확인」 36개다.** 대부분 폰 실물 조작이나 운영 실계정이라 사람만 밟을 수
> 있다. 그중 **차단형(`★`)이 12개**이고, 절반(6개)이 `oci-deploy`에 몰려 있다 — 백업은 배포 이후 지금까지
> 한 번도 검증된 적 없는 재해 복구다. 나머지 24개는 비차단형이라 `status`를 막지 않는다
> (`docs/conventions/verification.md`).
>
> **스펙 24개 중 22개가 `구현완료`다.** `초안` 둘(`test-login`·`web-follow`)은 **코드가 아니라 운영 검증을
> 기다린다** — 운영 `.env`에 `KALDI_TEST_LOGIN_SECRET`(32자 이상)을 넣으면 계정 둘로 맞팔로우를 밟을 수
> 있고, 그러면 둘을 한 번에 올릴 수 있다.
...
│   ├── specs/                ★ 기능 스펙 — AC를 가진 문서만 여기 둔다 (24개, AC 637개)
│   │   ├── TEMPLATE.md
│   │   └── YYYY-MM-DD-<기능>.md   날짜순. 22개가 `구현완료`, 둘은 운영 검증 대기라 `초안`
│   ├── plans/                구현 계획 (태스크 단위, 24개 — 전부 구현 완료)
│   │   ├── TEMPLATE.md
...
**기능 단위 스펙 여러 개**로 쪼개져 진행됐다(`docs/specs/`에 24개, `docs/plans/`에 24개).
...
> **주의:** 배포된 것이 곧 검증된 것은 아니다. **수동 확인 36개가 남아 있다**(차단형 12개).
> **로컬 브라우저로 밟을 수 있는 것은 2026-09-05에 전부 밟았다.** 남은 것은 도구가 아니라
> **폰·실계정·SSH 키**가 있어야 한다 — 가장 값싼 다음 수는 운영 `.env`에 테스트 로그인 시크릿을
> 넣는 것이다. 스펙 둘이 거기서 풀린다.
