...
import { hoffmannSummary, kasuyaSummary, pageOf, summaries } from '@/test/fixtures';
...
describe('RecipesPage', () => {
  it('AC-WEB-09 · 카드에 추출 파라미터가 표시된다', async () => {
    server.use(http.get(LIST_URL, () => HttpResponse.json(pageOf([hoffmannSummary]))));
  ...
  it('AC-WEB-10 · hasNext가 true면 더 보기 버튼이 있다', async () => {
    server.use(
      http.get(LIST_URL, () =>
        HttpResponse.json(pageOf(summaries(20), { hasNext: true, totalElements: 40 })),
      ),
    );
  ...
  it('AC-WEB-11 · hasNext가 false면 더 보기 버튼이 없다', async () => {
    server.use(
      http.get(LIST_URL, () => HttpResponse.json(pageOf([hoffmannSummary, kasuyaSummary]))),
    );
    ...
    server.use(
      http.get(LIST_URL, ({ request }) => {
        ...
        return HttpResponse.json(
          page === 0
            ? pageOf(summaries(20, 100), { page: 0, hasNext: true, totalElements: 40 })
            : pageOf(summaries(20, 200), { page: 1, hasNext: false, totalElements: 40 }),
        );
      }),
    );
