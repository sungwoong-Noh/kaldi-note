...
>
> **남은 것은 코드가 아니라 「수동 확인」 29개다.** 대부분 폰 실물 조작이나 SSH 키가 필요해 사람만 밟을 수
> 있다. 그중 **차단형(`★`)이 7개**이고, **6개가 `oci-deploy`에 몰려 있다** — 백업은 배포 이후 지금까지
> 한 번도 검증된 적 없는 재해 복구다. 나머지 22개는 비차단형이라 `status`를 막지 않는다
> (`docs/conventions/verification.md`).
>
> **가장 값싼 다음 수는 `media-attachment`의 차단형 하나다.** 그것이 막혀 있던 이유가 「운영에 로그인할
> 수단이 없다」였는데, 이제 테스트 로그인으로 풀린다.
...
> **주의:** 배포된 것이 곧 검증된 것은 아니다. **수동 확인 29개가 남아 있다**(차단형 7개).
> 브라우저로 밟을 수 있는 것은 전부 밟았다 — 2026-09-06에는 **운영**에서 테스트 로그인과 맞팔로우까지
> 밟았다. 남은 차단형 7개 중 **6개가 `oci-deploy`**(백업·롤백 실연습)이고 **SSH 키**가 필요하다.
> 나머지 하나(`media-attachment`의 OCI 실연동)는 이제 테스트 로그인으로 밟을 수 있다.
