...
> **현재 상태: 인터넷에 떠 있고 폰에서 쓸 수 있다.** 백엔드는 OCI VM에(`https://api.kaldi-note.today`),
> 프론트는 Cloudflare Workers에(`https://kaldi-note.today`) 배포돼 있다. 백엔드 테스트 482개 · 프론트 283개 ·
> 인수 조건 604개가 초록이다.
>
> **남은 것은 코드가 아니라 「수동 확인」 21개다.** 대부분 폰 실물 조작이나 운영 실계정이라 사람만 밟을 수 있다.
> 그중 **차단형(`★`)이 7개** — 백업 4개는 배포 이후 지금까지 한 번도 검증된 적 없는 재해 복구다.
> 나머지 14개는 비차단형이라 `status`를 막지 않는다(`docs/conventions/verification.md`).
...
│   ├── specs/                ★ 기능 스펙 — AC를 가진 문서만 여기 둔다 (22개, AC 604개 — 전부 커버리지 검사 대상)
│   │   ├── TEMPLATE.md
│   │   └── YYYY-MM-DD-<기능>.md   날짜순. `초안`은 없다 — 22개 모두 `구현완료`
│   ├── plans/                구현 계획 (태스크 단위, 22개)
│   │   ├── TEMPLATE.md
...
**기능 단위 스펙 여러 개**로 쪼개져 진행됐다(`docs/specs/`에 22개, `docs/plans/`에 22개).
...
| **4 — Frontend** | ✅ 완료 | Next.js PWA — 로그인·레시피 CRUD·브루잉 로그 CRUD·분쇄도 환산·앱 셸 |

**새 기능은 이제 이 표가 아니라 개별 스펙으로 시작한다.** `/interview` → `docs/specs/` → `docs/plans/` → 코드.
「Plan 2를 이어서 한다」 같은 단위는 더 이상 없다.

> **주의:** 배포된 것이 곧 검증된 것은 아니다. **수동 확인 21개가 남아 있다** — 운영·인프라 8개
> (백업이 실제로 도는지 아무도 확인한 적 없다), 운영 화면·실계정 5개, 폰 실물 조작 8개다.
> **로컬 브라우저로 밟을 수 있는 것은 2026-09-05에 전부 밟았다**(10개). 남은 것은 도구가 아니라
> 폰·실계정·SSH 키가 있어야 한다.
