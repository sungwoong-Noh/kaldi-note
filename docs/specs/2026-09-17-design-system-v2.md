...
그대로 허용하면 `AC-DS2-10`이 아무것도 막지 못한다. 인접값을 역할별로 묶어 **10단계로 확정한다.**

| 이름 | 폰트 | px | 디자인이 쓴 값 |
|---|---|---|---|
| `display` | Sans | **36** | 30 · 32 · 36 · 38 · 40 · 44 |
| `page-title` | Sans | **27** | 25 · 26 · 27 · 28 |
| `title` | Sans | **24** | 20 · 21 · 22 · 24 |
| `card-title` | Sans | **18** | 17 · 18 · 19 |
| `body` | Sans | **15** | 15 · 15.5 |
| `body-sm` | Sans | **13** | 13 · 13.5 |
| `caption` | Sans | **12** | 11.5 · 12 · 12.5 |
| `metric-hero` | **Mono** | **36** | 36 |
| `metric` | **Mono** | **14** | 14 · 14.5 · 15 · 16 |
| `label` | **Mono** | **10** | 9 · 9.5 · 10 · 10.5 · 11 |

**이 10개는 모바일 기준이다.** 웹 전용 크기(W1의 수율 44px·제목 32px)와 타이머의 76–84px은
각자의 스펙(`web-desktop`·`pour-timer`)이 추가한다. **여기서 미리 넣지 않는다** —
쓰이지 않는 단계는 `AC-DS2-11`(「10단계가 각각 쓰인다」)에 걸린다.
...
| `AC-SPACE-04` · `05` · `06` | 면 `rounded-lg`, 컨트롤 `rounded-md` | **4종** (아래 `AC-DS2-14`) |
...
#### AC-DS2-19 · 프리미티브가 터치 타깃 44px을 보장한다
...
#### AC-DS2-20 · 오류 상태 입력의 보더가 `danger`다
...
#### AC-DS2-21 · 오류 상태 입력에 `aria-invalid="true"`가 붙는다
...
#### AC-DS2-22 · 오류가 없으면 `aria-invalid`가 붙지 않는다
...
#### AC-DS2-24 · 팔레트 색 클래스와 임의값 색이 없다
...
| 폼 에러의 시각 형태 | **입력 보더 + `aria-invalid` + 문구** — `AC-DS2-20`~`22` |
