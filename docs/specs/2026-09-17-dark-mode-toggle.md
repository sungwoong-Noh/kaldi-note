...
#### AC-THEME-01 · 더보기 화면에 토글이 있다

- **Given** `/more` 화면
- **When** 렌더한다
- **Then** "다크 모드로 전환" 또는 "라이트 모드로 전환" 텍스트를 가진 버튼이 있다
...
- **Given** 시스템이 라이트고 저장된 선택이 없다 (현재 표시 = 라이트)
- **When** 토글 버튼을 누른다
- **Then** `<html data-theme="dark">`가 되고, 버튼 라벨이 "라이트 모드로 전환"으로 바뀐다
...
- **Given** 저장된 선택이 `"dark"`다
- **When** 토글을 누른다
- **Then** `localStorage`의 값이 `"light"`로 바뀌고 `<html data-theme="light">`다

#### AC-THEME-07 · 토글의 터치 영역이 44×44 이상이다

- **Given** `/more` 화면의 토글 버튼
- **When** 렌더된 높이를 잰다
- **Then** 44px 이상이다
