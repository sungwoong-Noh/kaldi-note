...
- **간격은 6단계뿐이다** — `1`(4px) · `2`(8px) · `3`(12px) · `4`(16px) · `6`(24px) · `12`(48px).
  `gap`·`space`·`p*`·`m*` 전부에 적용된다. **임의값과 음수 마진은 금지다.**
- **모서리는 역할로 정해진다** — **면**(카드·다이얼로그·스텝 카드)은 `rounded-lg`(8px),
  **컨트롤**(버튼·링크·입력칸·`select`·배지)은 `rounded-md`(6px). `rounded-full`은 진짜 원형
  (아바타·스피너)에만 쓴다. **`rounded`(4px)는 쓰지 않는다.**
- **그림자를 쓰지 않는다.** 다크 배경에서 거의 보이지 않아 값을 두 벌 잡아야 하고, 이 앱은
  `border-line`으로 면을 가른다. **0곳인 것이 결정이다.**
  → 위 셋은 `src/test/spacing.test.ts`와 `e2e/spacing.spec.ts`가 검사한다.
...
### ★ 픽스처는 실제 응답에서 뜬다. 지어내지 않는다

**MSW로 모킹한다는 것은 응답을 상상해도 된다는 뜻이 아니다.** 모킹하는 순간 테스트가 검증하는 대상은 백엔드가 아니라 **내가 적어둔 픽스처**가 된다. 픽스처가 실제와 다르면 테스트는 코드가 아니라 내 가정을 검증한다.

실제로 이 함정에 빠진 적이 있다 — 스텝 픽스처에 있지도 않은 `id`를 넣고 서버가 주는 `cumulativeWaterG`를 빠뜨렸더니, **테스트 54개가 전부 초록인데 상세 화면은 열리지 않았다**(`docs/archive/JOURNAL.md` 2026-08-21).

새 엔드포인트의 픽스처를 만들 때는 **반드시 실행 중인 백엔드의 응답을 떠온다.**

```bash
# 백엔드를 띄우고 실제 응답을 받아 그대로 옮긴다
curl -s -H "Authorization: Bearer $TOKEN" localhost:8080/api/v1/recipes/1 | python3 -m json.tool
```

- 필드를 **더하지도 빼지도 않는다.** 서버가 안 주는 필드를 픽스처에 넣으면 스키마가 그걸 요구하게 되고, 실제 응답에서 파싱이 깨진다.
- **`null`인 필드는 키 자체가 없다.** 백엔드가 `default-property-inclusion: non_null`이라 `"x": null`이 아니라 `x`가 아예 없다. Zod는 `.nullable()`이 아니라 `.optional()`로 쓴다.
- 픽스처 파일에 **어디서 떠온 것인지 주석으로 남긴다.** 다음 사람이 그대로 믿어도 되는지 판단할 수 있어야 한다.
