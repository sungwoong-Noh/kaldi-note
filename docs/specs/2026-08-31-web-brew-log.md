...
- [x] 그라인더도 원두도 없는 상태에서 시작해 모달 둘로 채우고 저장까지 간다
- [x] 저장한 로그가 홈과 목록에 나타나는지 본다
- [ ] 운영(`kaldi-note.today`)에서 기록·조회·삭제가 동작하는지 확인한다

> **2026-09-05 확인.** 4개 중 **2개를 켰다.** 남은 둘은 폰 실물과 운영 실계정이다.
>
> **✅ 빈손으로 시작해 모달 둘로 채우고 저장까지 갔다.** 기존 계정 둘(11·12)은 이미 그라인더를 갖고
> 있어 이 조건이 성립하지 않았다. 그래서 **로컬에 장비도 원두도 없는 계정(id 13 `빈손확인용`)을 새로
> 만들어** 밟았다. 기록 화면의 두 선택 상자가 실제로 `선택 안 함` 하나뿐이었고,
> 「+ 원두 등록」(로스터 새로 만들기 · 제품명 · 배전도 · 원산지 · 중량 · 로스팅일)과
> 「+ 그라인더 등록」(모델 · 별명)을 차례로 채우자 **등록 직후 그 항목이 선택 상자에 추가되고 자동으로
> 선택됐다** — 모달을 닫고 다시 고를 필요가 없다. 분쇄도 22 · 추출 시간 210초 · 별점 5로 저장까지 갔다.
>
> **✅ 홈과 목록에 나타난다.** 저장된 기록(`/brews/10`)이 홈 「최근 기록」과 `/brews` 목록 양쪽에
> 링크로 잡혔다. 앞서 계정 11로 만든 기록(`/brews/9`)도 같았다.
>
> **주의 — 이 확인을 다시 밟으려면 계정을 또 만들어야 한다.** id 13은 이제 원두와 그라인더를 가졌으므로
> 더는 「빈손」이 아니다.
...
> **2026-09-05 — 비차단형만 남아 `구현완료`로 올렸다.** 남은 4개는 전부 **폰에서의 작성 흐름 감각과 운영 눈 확인**이고,
> 같은 동작은 자동 테스트가 이미 덮는다(`docs/conventions/verification.md`「수동 확인은 두 종류다」).
> **상자는 비워 둔 채다 — 사람이 폰으로 밟으면 그때 켠다.** 올렸다고 없어진 것이 아니다.
> 초안으로 두면 이 스펙의 AC 46개가 커버리지 검사에서 함께 멈춘다.
