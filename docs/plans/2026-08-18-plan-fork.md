...
- [ ] 스펙(`docs/specs/2026-08-18-recipe-fork.md`)의 `status`를 `구현완료`로 변경
- [ ] Swagger UI에서 `POST /api/v1/recipes/{id}/fork`가 요청 본문 없이 실행되고, 응답에 `parentRecipeId`·`forkRootId`가 보인다 (스펙의 "수동 확인" 항목)
